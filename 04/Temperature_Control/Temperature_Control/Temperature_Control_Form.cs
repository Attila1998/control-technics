...
namespace Temperature_Control
{
    public partial class Temperature_Control_Form : Form
    {
        ...
        private void controlTimer_Tick(object sender, EventArgs ea)
        {
            ...
            double delta=0; /// a kesobbiekben majd beolvasva a frombol
                            /// a doboz mar kirakva
           

            /// ADC beolvasas
            short Analog_bemenet_0;
            Kartya.AIn(0, Range.Bip10Volts, out Analog_bemenet_0); /// beolvasom az analog bemenetet
            ...
            /// Skalazas
            double bemenet_voltban = (20 / 4095.0) * (Convert.ToDouble(Analog_bemenet_0) - 2048);
            double y_t = 100 * Analog_bemenet_0 - 273.15;
            ...
            else
            {
                if(Automat_radioButton.Checked) /// ha aktiv, szabalyzas
                {
                    ...
                    if (!HISZTEREZIS.Checked && !PVM.Checked)
                    {
                    ...
                    if(PVM.Checked)
                    {    
                        double kp = 25; //proporcionális erősítés
                        if(P.Checked)
                        {
                            PD.Checked = false;
                            u = kp * hiba;
                        ...
                        if (PD.Checked)
                        {
                            double Td = 0.01;
                            P.Checked = false;
                            int Ts = 1000; /// mintavetelezesi periodus
                            u = kp * (hiba + Td * ((hiba - hiba_regi) / Ts));
                            
                            
                        }
                        if(!P.Checked && !PD.Checked)
                        {
                            P.Checked = true;
                        }
                        ...
                        ControlSignal_textBox.Text = u.ToString("F");
                    }
            ...
            ControlSignal.Points.Add(u);  ///most  
        ...
        private void PWM_TIMER_Tick(object sender, EventArgs e)
        {
            if(Automat_radioButton.Checked)
            {
                ...
                {
                    PWM_Counter = 0;
                    PWM_Kitoltes = Math.Abs(Convert.ToInt32(u)); // a math.abs az elojel miatt
                }
                if(u>0) /// Futes
                {
                    if(PWM_Counter<PWM_Kitoltes)
                    {
                        Kartya.DOut(DigitalPortType.FifthPortA, 1);
                    }
                    else
                    {
                        Kartya.DOut(DigitalPortType.FifthPortA, 0);
                    }
                }
                else   /// hutes
                {
        ...
        private void ControlSignal_textBox_TextChanged(object sender, EventArgs e)
        {
